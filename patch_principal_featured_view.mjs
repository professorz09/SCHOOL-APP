...
c = c.replace(
  "import { ArrowLeft, Users, UserCheck, BookOpen, Receipt, Bus, CircleAlert, Wallet, ChevronRight, CheckCircle2 } from 'lucide-react';",
...
// update title logic and button
c = c.replace(
  `let title = '';
  switch (feature) {
    case 'STUDENTS': title = 'Students Directory'; break;`,
  `let title = '';
  switch (feature) {
    case 'STUDENTS': 
...
// Handle Replacement for the STUDENTS feature
const STUDENTS_OLD = `{feature === 'STUDENTS' && (
           <div className="space-y-4">
               <AppCard noPadding className="shadow-sm border border-slate-100 mb-4">
                 <div className="divide-y divide-slate-100">
                   {[
                     { name: 'Aarav Sharma', cls: '10-A', roll: '45', status: 'Present' },
                     { name: 'Isha Patel', cls: '10-A', roll: '12', status: 'Absent' },
                     { name: 'Rohan Gupta', cls: '9-B', roll: '33', status: 'Present' },
                     { name: 'Ananya Singh', cls: '9-B', roll: '05', status: 'Present' }
                   ].map((student, i) => (
                     <div key={i} className="p-4 flex items-center justify-between hover:bg-slate-50 transition-colors cursor-pointer">
                       <div className="flex items-center gap-3">
                         <div className="w-10 h-10 rounded-full bg-indigo-100 flex items-center justify-center font-bold text-lg text-indigo-700 uppercase">
                           {student.name.charAt(0)}
                         </div>
                         <div>
                           <span className="font-extrabold text-slate-900 text-sm tracking-tight block">{student.name}</span>
                           <span className="font-bold text-slate-500 text-[10px] uppercase tracking-widest">{student.cls} • Roll: {student.roll}</span>
                         </div>
                       </div>
...
const STUDENTS_NEW = `{feature === 'STUDENTS' && (
...
c = c.replace(STUDENTS_OLD, STUDENTS_NEW);
