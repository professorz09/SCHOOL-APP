...
export const TimetableView: React.FC<Props> = ({ onBack }) => {
  ...
  return (
    <div className="w-full bg-slate-50 flex flex-col animate-in slide-in-from-right-8 duration-300">
      {/* Header */}
      <div className="bg-white border-b border-slate-100 px-4 lg:px-8 pt-4 lg:pt-6 pb-0 sticky top-0 z-10 shadow-sm">
        <div className="flex items-center gap-3 pb-3 lg:pb-4 lg:max-w-5xl lg:mx-auto lg:w-full">
          <button onClick={onBack} className="p-2 -ml-2 bg-slate-100 rounded-full text-slate-600 hover:bg-slate-200 transition-colors">
            <ArrowLeft size={20} />
          </button>
          <div>
            <h2 className="text-xl lg:text-2xl font-black text-slate-900 uppercase tracking-tight">Timetable</h2>
            <p className="text-[10px] lg:text-xs font-bold text-slate-400">Weekly Schedule</p>
          </div>
        </div>

        {/* Day tabs */}
        <div className="flex border-t border-slate-100 overflow-x-auto hide-scrollbar lg:max-w-5xl lg:mx-auto lg:w-full lg:gap-2">
          {DAY_TABS.map(day => (
            <button key={day} onClick={() => setActiveDay(day)}
              className={`shrink-0 flex flex-col items-center px-3 lg:px-5 py-2.5 lg:py-3 border-b-2 transition-colors ${
                activeDay === day ? 'border-blue-600 text-blue-600' : 'border-transparent text-slate-400 hover:text-slate-600'
              }`}>
              <span className={`text-[10px] lg:text-xs font-black uppercase tracking-widest ${day === today ? 'text-emerald-500' : ''}`}>
                <span className="lg:hidden">{day.slice(0, 3)}</span>
                <span className="hidden lg:inline">{day}</span>
              </span>
              {day === today && (
                <span className="text-[8px] lg:text-[9px] font-black text-emerald-500">Today</span>
              )}
            </button>
          ))}
        </div>
      ...
      {/* Legend */}
      <div className="px-4 lg:px-8 py-2.5 lg:py-3 bg-white border-b border-slate-100">
        <div className="flex gap-3 lg:gap-5 overflow-x-auto hide-scrollbar lg:max-w-5xl lg:mx-auto lg:w-full">
          {([['CLASS', 'bg-blue-500'], ['LUNCH', 'bg-amber-400'], ['ASSEMBLY', 'bg-violet-500'], ['EXAM', 'bg-rose-500']] as [string, string][]).map(([type, dot]) => (
            <div key={type} className="flex items-center gap-1.5 shrink-0">
              ...
              <span className="text-[9px] lg:text-[10px] font-black uppercase tracking-widest text-slate-500">{type}</span>
            </div>
          ))}
        </div>
      </div>
