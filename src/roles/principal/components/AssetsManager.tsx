...
export const AssetsManager: React.FC<Props> = ({ onBack }) => {
  ...
  const [bookHistory, setBookHistory] = useState<AssetHistoryRow[]>([]);
  const [eqHistory, setEqHistory] = useState<AssetHistoryRow[]>([]);
  ...
  const [returnModal, setReturnModal] = useState<{
    bookId: string; studentId: string; studentName: string; bookTitle: string;
  } | null>(null);
  ...
  // Refresh issue history from DB. Called after issue/return + on tab switch.
  const refreshHistory = React.useCallback(async () => {
    try {
      const [bh, eh] = await Promise.all([
        principalService.getAssetHistory('BOOK'),
        principalService.getAssetHistory('LAB_EQUIPMENT'),
      ]);
      setBookHistory(bh);
      setEqHistory(eh);
    } catch (e) {
      showToast(e instanceof Error ? e.message : 'Failed to load asset history', 'error');
    }
  }, [showToast]);
  ...
  const handleIssueBook = async () => {
    ...
    setIsSubmitting(true);
    try {
      const updated = await principalService.issueBook(
        issueModal.id, student.id, student.name, issueNote,
      );
  ...
  const handleConfirmReturn = async () => {
    ...
    setIsSubmitting(true);
    try {
      const updated = await principalService.returnBook(
        returnModal.bookId, returnModal.studentId, returnNote,
      );
      setBooks(prev => prev.map(b => b.id === updated.id ? updated : b));
      await refreshHistory();
      showToast(`Book returned by ${returnModal.studentName}`);
      setReturnModal(null);
      setReturnNote('');
    } catch (e) {
      showToast(e instanceof Error ? e.message : 'Return failed', 'error');
    } finally { setIsSubmitting(false); }
  };
  ...
  return (
    <div className="w-full bg-slate-50 flex flex-col animate-in slide-in-from-right-8 duration-300">
      ...
      <div className="flex-1 overflow-y-auto p-4  space-y-3">
                  ...
                  <div className="border-t border-slate-50 pt-2 mt-2 space-y-1.5">
                    {book.issuedTo.filter(i => !i.returnedAt).map(issue => (
                      <div key={issue.studentId} className="flex items-center justify-between">
                        ...
                        <button onClick={() => {
                            setReturnModal({
                              bookId: book.id, studentId: issue.studentId,
                              studentName: issue.studentName, bookTitle: book.title,
                            });
                            setReturnNote('');
                          }}
        ...
        {/* LIBRARY HISTORY — DB-backed loan log */}
        {tab === 'LIBRARY' && librarySubTab === 'HISTORY' && (
          <>
            {bookHistory.length === 0 ? (
              <div className="flex flex-col items-center py-16 text-slate-400">
                <HistoryIcon size={32} className="mb-3 opacity-40" />
                <p className="font-bold text-sm">No book loans yet</p>
              </div>
            ) : (
              bookHistory.map(h => (
                <div key={h.id} className="bg-white rounded-2xl border border-slate-100 shadow-sm p-4">
                  <div className="flex items-start justify-between gap-2">
                    <div className="flex-1">
                      <div className="font-extrabold text-slate-900 text-sm">{h.assetName}</div>
                      <div className="text-[10px] font-bold text-slate-400 mt-1">
                        {h.returnedAt ? `Returned by ${h.borrowerName}` : `Issued to ${h.borrowerName}`}
                      </div>
                      <div className="text-[10px] font-bold text-slate-400 mt-0.5">
                        Issued: {h.issuedAt}{h.dueDate ? ` · Due: ${h.dueDate}` : ''}
                        {h.returnedAt ? ` · Returned: ${h.returnedAt}` : ''}
                      </div>
                      {(h.issuedByName || h.returnedByName) && (
                        <div className="text-[10px] font-bold text-slate-500 mt-1">
                          {h.issuedByName && <>By: {h.issuedByName}</>}
                          {h.returnedByName && <> · Closed by: {h.returnedByName}</>}
                        </div>
                      )}
                      {h.issueNote && (
                        <div className="text-[10px] font-bold text-slate-600 bg-slate-50 rounded-lg px-2 py-1 mt-2">
                          <span className="text-slate-400">At issue:</span> {h.issueNote}
                        </div>
                      )}
                      {h.returnNote && (
                        <div className="text-[10px] font-bold text-emerald-700 bg-emerald-50 rounded-lg px-2 py-1 mt-1">
                          <span className="text-emerald-500">At return:</span> {h.returnNote}
                        </div>
                      )}
                    </div>
                    <div className="text-right shrink-0">
                      <div className={`text-[9px] font-black px-2 py-1 rounded-full ${h.returnedAt ? 'bg-emerald-50 text-emerald-700' : 'bg-amber-50 text-amber-700'}`}>
                        {h.returnedAt ? 'RETURNED' : 'ISSUED'}
                      </div>
                    </div>
                  </div>
                </div>
        ...
        {/* LAB HISTORY — DB-backed equipment loan log */}
        {tab === 'LAB' && labSubTab === 'HISTORY' && (
          <>
            {eqHistory.length === 0 ? (
              <div className="flex flex-col items-center py-16 text-slate-400">
                <HistoryIcon size={32} className="mb-3 opacity-40" />
                <p className="font-bold text-sm">No equipment loans yet</p>
              </div>
            ) : (
              eqHistory.map(h => (
                <div key={h.id} className="bg-white rounded-2xl border border-slate-100 shadow-sm p-4">
                  <div className="flex items-start justify-between gap-2">
                    <div className="flex-1">
                      ...
                      <div className="text-[10px] font-bold text-slate-400 mt-0.5">
                        Issued: {h.issuedAt}{h.returnedAt ? ` · Returned: ${h.returnedAt}` : ''}
                      </div>
                      {(h.issuedByName || h.returnedByName) && (
                        <div className="text-[10px] font-bold text-slate-500 mt-1">
                          {h.issuedByName && <>By: {h.issuedByName}</>}
      ...
      {issueModal && (
        <div className="absolute inset-0 bg-slate-900/50 backdrop-blur-sm z-50 flex items-end">
          <div className="w-full bg-white rounded-t-3xl p-5 pb-8 animate-in slide-in-from-bottom-8 space-y-4">
            ...
            <div>
              <label className="block text-[10px] font-black uppercase tracking-widest text-slate-400 mb-1.5">Condition / Note (optional)</label>
              <textarea value={issueNote} onChange={e => setIssueNote(e.target.value)}
                rows={2} placeholder="e.g. Cover slightly torn, pages intact"
      ...
      {/* Return Book Modal — captures returner's condition note */}
      {returnModal && (
        <div className="absolute inset-0 bg-slate-900/50 backdrop-blur-sm z-50 flex items-end">
          <div className="w-full bg-white rounded-t-3xl p-5 pb-8 animate-in slide-in-from-bottom-8 space-y-4">
            <div className="flex items-center justify-between">
              <div>
                <h3 className="font-black text-slate-900">Return Book</h3>
                <p className="text-xs font-bold text-slate-400 mt-0.5">{returnModal.bookTitle}</p>
                <p className="text-[10px] font-bold text-slate-500 mt-0.5">From: {returnModal.studentName}</p>
              </div>
              <button onClick={() => setReturnModal(null)} className="w-8 h-8 bg-slate-100 rounded-full flex items-center justify-center">
                <X size={16} className="text-slate-500" />
              </button>
            </div>
            <div>
              <label className="block text-[10px] font-black uppercase tracking-widest text-slate-400 mb-1.5">Return Condition (optional)</label>
              <textarea value={returnNote} onChange={e => setReturnNote(e.target.value)}
                rows={3} placeholder="e.g. Returned in good condition; minor wear on spine"
            ...
            <button onClick={handleConfirmReturn} disabled={isSubmitting}
              className="w-full flex items-center justify-center gap-2 bg-emerald-600 text-white font-black text-sm uppercase tracking-widest py-4 rounded-2xl active:scale-95 transition-transform disabled:opacity-60">
              {isSubmitting ? 'Returning…' : <><RotateCcw size={16} /> Confirm Return</>}
            </button>
          </div>
        </div>
      )}
