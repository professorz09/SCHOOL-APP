...
import { ArrowLeft, Bus, MapPin, CheckCircle2, Circle, Navigation, Phone } from 'lucide-react';
import { studentDashboardService } from '../../../services/student.service2';
import { TransportStop } from '../../../types/student.types';
...
export const TransportView: React.FC<Props> = ({ onBack }) => {
  ...
  useEffect(() => {
    studentDashboardService.getTransportStops().then(setStops);
        ...
        <div className="bg-slate-900 rounded-2xl p-4 text-white">
          <div className="flex items-center justify-between mb-3">
            <div className="flex items-center gap-2">
              ...
              <span className="font-black text-white">DL-01-CA-1234</span>
            </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              ...
              <div className="font-black text-sm text-white mt-0.5">Rajan Kumar</div>
            </div>
            <div>
              <div className="text-[9px] font-black uppercase tracking-widest text-slate-400">Your ETA</div>
              <div className="font-black text-sm text-emerald-300 mt-0.5">~8:35 AM</div>
            </div>
          </div>
        </div>

        {/* Call driver */}
        <a href="tel:+919800166666"
          ...
          <Phone size={16} className="text-emerald-500" /> Call Driver: +91 98001 66666
        </a>

        {/* Route stops */}
        <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-4">
          <p className="text-[10px] font-black uppercase tracking-widest text-slate-400 mb-4">Route A — Live Stops</p>
          <div className="space-y-1">
            {stops.map((stop, i) => (
              <div key={i} className="flex gap-4 items-start">
                ...
                <div className={`flex-1 pb-4 ${i === stops.length - 1 ? 'pb-0' : ''}`}>
                  ...
                  {stop.status === 'CURRENT' && (
                    <span className="text-[9px] font-black bg-blue-500 text-white px-2 py-0.5 rounded-full uppercase mt-1 inline-block">Van is here</span>
                  )}
                </div>
              </div>
            ))}
