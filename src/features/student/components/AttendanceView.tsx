...
type DayStatus = 'PRESENT' | 'ABSENT' | 'HOLIDAY' | 'HALF_DAY';
...
const MONTH_DATA: MonthSummary[] = [
  { month: 'Apr 2026', present: 18, absent: 2, holiday: 4, total: 24 },
  { month: 'Mar 2026', present: 22, absent: 1, holiday: 4, total: 27 },
  { month: 'Feb 2026', present: 19, absent: 3, holiday: 2, total: 24 },
  { month: 'Jan 2026', present: 21, absent: 2, holiday: 4, total: 27 },
  { month: 'Dec 2025', present: 17, absent: 1, holiday: 6, total: 24 },
  { month: 'Nov 2025', present: 20, absent: 4, holiday: 2, total: 26 },
  { month: 'Oct 2025', present: 23, absent: 1, holiday: 3, total: 27 },
  { month: 'Sep 2025', present: 21, absent: 2, holiday: 3, total: 26 },
];
...
export const AttendanceView: React.FC<Props> = ({ onBack }) => {
  ...
  const totalPresent  = MONTH_DATA.reduce((a, m) => a + m.present, 0);
  const totalAbsent   = MONTH_DATA.reduce((a, m) => a + m.absent, 0);
  const totalWorkDays = MONTH_DATA.reduce((a, m) => a + m.present + m.absent, 0);
          ...
          <div className="flex items-end gap-4 mb-4">
            ...
            <div className="pb-1">
              <div className="text-sm font-black text-blue-100">
                {overallPct >= 90 ? 'Excellent' : overallPct >= 75 ? 'Good Standing' : 'Needs Improvement'}
              </div>
          ...
          {overallPct < 75 && (
            <div className="mt-3 bg-rose-500/20 border border-rose-400/30 rounded-xl px-3 py-2">
              <p className="text-[10px] font-black text-rose-300 uppercase tracking-wide">
                ⚠️ Attendance below 75% — risk of being barred from exams
              </p>
            </div>
          )}
        </div>
