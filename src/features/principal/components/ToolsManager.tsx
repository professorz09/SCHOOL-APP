...
export const ToolsManager: React.FC<Props> = ({ onBack }) => {
  ...
  const [schoolInfo] = React.useState(() => schoolInfoService.get());
  ...
  const QuestionPaperGenerator = () => {
    ...
    const handleGenerate = async () => {
      ...
      setTimeout(() => { setGenerated(true); setIsGenerating(false); }, 2000);
    };

    if (generated) {
      return (
        <div className="w-full flex flex-col">
          <ToolHeader title="Question Paper" onBackPress={() => setGenerated(false)} />
          <div className="p-5 space-y-4">
            <div className="bg-white border-2 border-slate-200 rounded-2xl p-6 shadow-sm">
              <div className="text-center mb-6 pb-4 border-b-2 border-slate-200">
                <h3 className="text-lg font-black text-slate-900">{config.subject.toUpperCase()}</h3>
                <p className="text-sm font-bold text-slate-500 mt-1">
                  Class {config.class}-{config.section} | Time: 3 Hours | Total Marks: {config.totalMarks}
                </p>
              </div>
                  ...
                  <div className="space-y-2">
                    {[1, 2, 3].map(i => (
                      <div key={i} className="text-sm font-bold text-slate-700">
                        ...
                        <div className="ml-4 text-xs text-slate-500 mt-1">a) 10  b) 12  c) 14  d) 16</div>
                      </div>
                    ))}
                  </div>
                </div>
        ...
        <div className="p-5 space-y-4">
          <div className="bg-purple-50 border border-purple-200 rounded-2xl p-4 flex items-start gap-3">
            ...
            <div>
              ...
              <p className="text-xs font-bold text-purple-700 mt-0.5">Generate question papers automatically with AI</p>
            </div>
          </div>
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div>
                <label className="text-xs font-black uppercase tracking-widest text-slate-400 mb-1.5 block">Questions</label>
                <input type="number" value={config.numQuestions}
