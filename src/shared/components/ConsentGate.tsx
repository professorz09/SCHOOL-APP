...
import { ShieldCheck, AlertCircle, Loader, Languages } from 'lucide-react';
...
export const ConsentGate: React.FC = () => {
  ...
  const handleAccept = async () => {
    ...
    try {
      ...
      showToast(lang === 'hi' ? 'Dhanyavaad — consent record ho gaya' : 'Thanks — consent recorded');
    } catch (err) {
      ...
      <div className="w-full max-w-lg bg-white rounded-3xl shadow-2xl flex flex-col overflow-hidden">
            ...
            <div className="flex items-center gap-3 min-w-0">
              ...
              <div className="min-w-0">
                <div className="text-xl font-black leading-tight">
                  {lang === 'hi' ? 'Data Consent' : 'Data Consent'}
                </div>
                <div className="text-[11px] font-bold text-emerald-100 mt-0.5">
                  {lang === 'hi' ? 'DPDP Act 2023 ke tahat' : 'Under the DPDP Act, 2023'}
                </div>
              </div>
            </div>
            <button
              onClick={() => setLang(l => (l === 'hi' ? 'en' : 'hi'))}
              className="shrink-0 px-3 py-1.5 rounded-lg bg-white/20 hover:bg-white/30 text-[11px] font-black uppercase tracking-wide flex items-center gap-1.5">
              <Languages size={13} />
              {lang === 'hi' ? 'EN' : 'हिं'}
            </button>
          </div>
        ...
        <div className="p-6 pb-7 max-h-[calc(100dvh-220px)] overflow-y-auto">
          ...
          <button
            ...
            {submitting
              ? <><Loader size={20} className="animate-spin" /> {lang === 'hi' ? 'Save ho raha…' : 'Saving…'}</>
              : (lang === 'hi' ? 'Agree karta hun' : 'I Agree')}
          </button>

          <button
            onClick={async () => { await logout(); }}
            className="w-full mt-3 py-3 text-slate-500 font-bold text-sm hover:text-slate-700 transition-colors">
            {lang === 'hi' ? 'Disagree karke logout' : 'Disagree & log out'}
          </button>
        </div>
      </div>
    </div>
